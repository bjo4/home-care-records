...
export default async function VisitsPage({ searchParams }: Props) {
  ...
  const edit = data.visits.find((visit) => visit.id === params?.edit);
  const visits = [...data.visits].sort((a, b) => b.date.localeCompare(a.date));

  return <AppFrame active="today" title="看診" description="整理科別、醫囑與下次回診日期。"><VisitForm user={user.displayName} edit={edit} /><VisitList visits={visits} /></AppFrame>;
}

function VisitForm({ user, edit }: { user: string; edit?: Awaited<ReturnType<typeof getCareLog>>["visits"][number] }) {

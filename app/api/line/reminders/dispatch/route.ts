...
import { buildDueRemindersFlexMessage, getLinePushTargets } from "@/lib/line";
...
export async function POST(request: NextRequest) {
  ...
  const message = buildDueRemindersFlexMessage(due);
  if (!message) return NextResponse.json({ pushed: 0, due: 0 });
  let pushed = 0;
  for (const to of getLinePushTargets(data)) {
    await fetch("https://api.line.me/v2/bot/message/push", { method: "POST", headers: { authorization: `Bearer ${channelToken}`, "content-type": "application/json" }, body: JSON.stringify({ to, messages: [message] }) });
    pushed += 1;
  }

...
export async function POST(request: NextRequest) {
  ...
  let pushed = 0;
  for (const binding of data.lineBindings) {
    if (due.length === 0) continue;
    await fetch("https://api.line.me/v2/bot/message/push", { method: "POST", headers: { authorization: `Bearer ${channelToken}`, "content-type": "application/json" }, body: JSON.stringify({ to: binding.lineUserId, messages: [{ type: "text", text: `CareLog 提醒：未來 48 小時有 ${due.length} 項待辦。` }] }) });
    pushed += 1;
  }

...
export default async function ExamsPage({ searchParams }: Props) {
  ...
  const edit = data.exams.find((exam) => exam.id === params?.edit);
  const exams = [...data.exams].sort((a, b) => b.datetime.localeCompare(a.datetime));

  return <AppFrame active="today" title="檢查" description="記錄檢查安排、結果摘要與下次追蹤。"><ExamForm user={user.displayName} edit={edit} /><ExamList exams={exams} /></AppFrame>;
}

function ExamForm({ user, edit }: { user: string; edit?: Awaited<ReturnType<typeof getCareLog>>["exams"][number] }) {
